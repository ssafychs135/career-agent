...
2. **n8n `03-notifier` 비활성화** — 검증보다 **먼저** 한다(아래 이유).
3. Ops에서 **지금 알림 발송**으로 수동 검증.
4. Ops에서 `알림 활성화` ON.

**왜 비활성화가 검증보다 먼저인가** — 최초 설계는 2/3이 반대였는데, 그러면 검증 자체가 이중 발송 창을 만든다. 현재 미전송이 0건이라 수동 검증에는 새 `done` 공고가 필요한데, n8n이 살아 있으면 5분 안에 그 공고를 소비해 버린다. 결국 아무것도 못 보거나, n8n의 SELECT→발송→UPDATE와 경합해 같은 공고를 두 번 보낸다. n8n을 먼저 끄더라도 `notify_enabled`가 여전히 false라 **명시적으로 버튼을 누르기 전까지 아무것도 발송되지 않으므로** 안전하다.

### 운영자가 알아야 할 것

- **수동 발송 버튼은 dry-run이 아니다.** 전역 필터에 걸린 공고는 발송 없이 `notified_at`이 찍혀 영구 소비된다. 첫 클릭 전에 `allowed_regions`/`hidden_companies` 실제 값을 확인할 것 — n8n은 전부 보냈으므로, 기대보다 엄격한 필터가 걸려 있으면 첫 검증에서 실제 공고가 조용히 삼켜진다.
- **실패 알림이 모든 파이프라인에서 처음으로 살아난다.** 웹훅 수정으로 `run_log`의 스케줄 실패 알림이 동작하기 시작하므로, 수집기·워커 실패도 이제 같은 채널로 온다. 배포 직후 그동안 안 보이던 알림이 늘 수 있다.
- **결정적으로 실패하는 청크는 큐를 막는다.** `ORDER BY collected_at LIMIT 30`이라 같은 선두 행을 계속 다시 집는다. 어떤 청크가 항상 실패하면(예: 임베드 합산 6000자 초과) 마킹되지 않아 5분마다 재시도되고, 뒤의 새 공고가 전부 막히며, 매 재시도가 실패 알림을 같은 채널로 보낸다. 차단 스위치는 Ops의 `알림 활성화` OFF(진행 중인 틱은 취소되지 않고 다음 틱부터 적용).
- 전환 확인 쿼리: `SELECT count(*) FROM jobs WHERE status='done' AND notified_at IS NULL;` — 몇 틱 안에 0 근처로 돌아와야 한다.
